...
contract PennyWhale is ERC20, Ownable {
    ...
    constructor(
    ...
    ) ERC20("PennyWhale", "PENNY") Ownable(_initialOwner) {
        ...
        // create BNB pair address
        bnbPairAddress = IPancakeFactory(pancakeRouter.factory()).createPair(
            address(wbnb),
            address(this)
        );

        // create USDT pair address
        usdtPairAddress = IPancakeFactory(pancakeRouter.factory()).createPair(
            usdtAddress,
            address(this)
        );
    ...
    // Add to Non Taxable Address
    function addToNonTaxableAddress(
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    // Remove from Non Taxable Address
    function removeFromNonTaxableAddress(
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    // Check if Address is Non Taxable Address
    function isNonTaxableAddress(
        address _address
    ) external view returns (bool) {
        return nonTaxableAddress[_address];
    }

    // Add to Taxable Address
    function addToTaxableAddress(
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    // Remove from Taxable Address
    function removeFromTaxableAddress(
        address[] calldata _addresses
    ) external onlyOwner {
        for (uint256 i = 0; i < _addresses.length; i++) {
    ...
    // Check if Address is Taxable Address
    function isTaxableAddress(address _address) external view returns (bool) {
        return taxableAddress[_address];
    }
    ...
    function _update(
        ...
        // If from or to is zero address, call the parent function
        if (from == address(0) || to == address(0)) {
            super._update(from, to, amount);
            return;
        }

        // Only allow transfers from BNB pair address to manager address
        if (from == bnbPairAddress) {
            require(
                to == managerAddress,
                "PennyWhale: only manager can interact with BNB pair"
            );
            super._update(from, to, amount);
            return;
        }

        // Only allow transfers to BNB pair address from manager address
        if (to == bnbPairAddress) {
            require(
                from == managerAddress,
                "PennyWhale: only manager can interact with BNB pair"
            );
        ...
        require(
            !blacklistedAddress[from] && !blacklistedAddress[to],
            "blacklisted address not allowed to transfer"
        );

        if (nonTaxableAddress[from] || nonTaxableAddress[to]) {
            super._update(from, to, amount);
            return;
        }

        if (taxableAddress[from] || taxableAddress[to]) {
            uint256 fee1p = (amount * 100) / 10_000;
            uint256 fee2p = (amount * 200) / 10_000;
            if (fee1p > 0) {
                super._update(from, feeWallet1p, fee1p);
                amount -= fee1p;
            }
            if (fee2p > 0) {
                super._update(from, feeWallet2p, fee2p);
                amount -= fee2p;
            }
        }
        super._update(from, to, amount);
    ...
    function setFeeAddress(
        address _feeAddress1p,
        address _feeAddress2p
    ) external onlyOwner {
        require(_feeAddress1p != address(0), "Invalid fee address");
